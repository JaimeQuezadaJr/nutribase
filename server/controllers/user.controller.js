...
const register = async (req, res) => {
  try {
    ...
    const userToken = jwt.sign(
      { _id: newUser._id, email: newUser.email, firstName: newUser.firstName, lastName: newUser.lastName, age: newUser.age },
      SECRET,
    );
  ...
  if (!userDocument) {
    ...
    try {
      ...
      if (!isPasswordValid) {
        ...
        const userToken = jwt.sign(
          { _id: userDocument._id, email: userDocument.email, firstName: userDocument.firstName, lastName: userDocument.lastName, age: userDocument.age },
          SECRET,
        );
        console.log('JWT:', userToken);
        res
